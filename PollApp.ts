...
import { createOrderModal } from './src/lib/createOrderModal';
...
export class PollApp extends App implements IUIKitInteractionHandler {
    ...
    public async executeBlockActionHandler(
        ...
        switch (actionId) {
            ...
            case 'join': {
                const modal = await createOrderModal({
                    data,
                    persistence,
                    modify,
                });
    ...
    public async initialize(
        ...
        await configuration.settings.provideSetting({
            id: 'use-user-name',
            i18nLabel:
                'Use name attribute to display voters, instead of username',
            i18nDescription:
                'When checked, display voters as full user names instead of username',
            required: false,
